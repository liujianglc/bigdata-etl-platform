...
def detect_and_fix_schema_issues():
    ...
    try:
        ...
        for partition in partitions:
            ...
            partition_path = f"hdfs://namenode:9000/user/hive/warehouse/dwd_db.db/dwd_orders/{partition}"
            ...
            try:
                ...
                if 'decimal(10,2)' in current_type.lower():
                    ...
                    logging.info(f"✅ 分区 {partition_date}: NetAmount类型正确 ({current_type})")
                else:
                    problem_partitions.append((partition_date, partition_path, current_type))
                    logging.warning(f"⚠️  分区 {partition_date}: NetAmount类型错误 ({current_type})")
                    
            except Exception as e:
                logging.error(f"❌ 检测分区 {partition_date} 时出错: {str(e)}")
                problem_partitions.append((partition_date, partition_path, "ERROR"))
        
        logging.info(f"\n📋 检测结果:")
        logging.info(f"  - 正常分区: {len(good_partitions)} 个")
        logging.info(f"  - 问题分区: {len(problem_partitions)} 个")
        
        if not problem_partitions:
            logging.info("🎉 所有分区的NetAmount列类型都正确!")
            return
        
        logging.info(f"\n🔧 开始修复 {len(problem_partitions)} 个问题分区...")
        ...
        for partition_date, partition_path, current_type in problem_partitions:
            logging.info(f"\n📅 处理分区: {partition_date}")
            logging.info(f"📂 分区路径: {partition_path}")
            logging.info(f"📊 当前类型: {current_type}")
            
            if current_type == "ERROR":
                # 如果读取时就出错，直接删除分区
                logging.info(f"🗑️  分区 {partition_date} 存在读取错误，删除并重新生成")
                try:
                    ...
                    failed_partitions.append(f"{partition_date} (已删除-读取错误)")
            ...
            try:
                ...
                df = spark.read.parquet(partition_path)
                row_count = df.count()
                logging.info(f"📊 分区行数: {row_count}")
                
                if row_count == 0:
                    logging.info(f"⚠️  分区 {partition_date} 为空，跳过修复")
                    successful_fixes += 1
                    continue
                ...
                for row in sample_fixed:
                    logging.info(f"  OrderID: {row['OrderID']}, NetAmount: {row['NetAmount']}")
                
                # 创建备份路径
                backup_path = f"{partition_path}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                
                # 备份原始分区
                logging.info(f"📦 备份原始分区到: {backup_path}")
                df.write \
                    .mode("overwrite") \
                ...
                if verify_count == row_count and 'decimal(10,2)' in fixed_type.lower():
                    ...
                    logging.info(f"  - 行数: {verify_count}")
                    logging.info(f"  - 类型: {fixed_type}")
                    successful_fixes += 1
                else:
                    logging.error(f"❌ 分区 {partition_date} 修复验证失败:")
                    logging.error(f"  - 原始行数: {row_count}, 修复后行数: {verify_count}")
                    logging.error(f"  - 修复后类型: {fixed_type}")
                    failed_partitions.append(partition_date)
                    
            except Exception as fix_error:
                logging.error(f"❌ 修复分区 {partition_date} 时出错: {str(fix_error)}")
                ...
                try:
                    spark.sql(f"ALTER TABLE dwd_db.dwd_orders DROP IF EXISTS PARTITION (dt='{partition_date}')")
                    logging.info(f"✅ 已删除有问题的分区 {partition_date}")
                    failed_partitions.append(f"{partition_date} (已删除-修复失败)")
                except Exception as drop_error:
                    logging.error(f"❌ 删除分区 {partition_date} 时出错: {str(drop_error)}")
                    failed_partitions.append(f"{partition_date} (删除失败)")
        ...
        # 总结修复结果
        logging.info(f"\n📋 修复总结:")
        logging.info(f"  - 检测到问题分区: {len(problem_partitions)} 个")
        logging.info(f"  - 成功修复: {successful_fixes} 个分区")
        logging.info(f"  - 失败/删除分区: {len(failed_partitions)} 个")
        ...
        # 提供后续建议
        logging.info(f"\n💡 后续建议:")
        logging.info(f"1. 重新运行失败的DWS分析作业")
        logging.info(f"2. 检查ETL管道确保新数据使用正确的类型转换")
        logging.info(f"3. 监控后续分区是否还会出现类似问题")
        
    except Exception as e:
        logging.error(f"❌ 检测和修复过程中出现错误: {str(e)}")
        raise
    finally:
        spark.stop()
